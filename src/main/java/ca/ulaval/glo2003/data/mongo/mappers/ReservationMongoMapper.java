...
public class ReservationMongoMapper {
    ...
    public ReservationMongo toMongo(Reservation reservation) {
        return new ReservationMongo(reservation.getNumber(), reservation.getDate().toString(),
                timeMapper.toMongo(reservation.getReservationTime()),
                        reservation.getGroupSize(), customerMapper.toMongo(reservation.getCustomer()), reservation.getRestaurantId());
    }

    public Reservation fromMongo(ReservationMongo reservation) {
        return new Reservation(reservation.number, LocalDate.parse(reservation.date),
                timeMapper.fromMongo(reservation.reservationTime),
                reservation.groupSize, customerMapper.fromMongo(reservation.customer), reservation.restaurantId);
    }
}
