...
public class RestaurantMongoMapper {
    ...
    public RestaurantMongo toMongo(Restaurant restaurant) {
        return new RestaurantMongo(restaurant.getId(), restaurant.getOwnerId(),
                restaurant.getName(),restaurant.getCapacity(), hoursMapper.toMongo(restaurant.getHours()), restaurant.getConfiguration().getDuration());
    }

    public Restaurant fromMongo(RestaurantMongo restaurant) {
        return new Restaurant(restaurant.id, restaurant.ownerId, restaurant.name,
                restaurant.capacity, hoursMapper.fromMongo(restaurant.hours), new RestaurantConfiguration(restaurant.reservationsDuration));
    }
}
