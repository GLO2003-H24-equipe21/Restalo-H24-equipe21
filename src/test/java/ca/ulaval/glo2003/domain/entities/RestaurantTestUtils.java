...
public class RestaurantTestUtils {
    public static List<Restaurant> createRestaurants(int number) {
        return IntStream.range(0, number)
                .mapToObj(i -> new Restaurant(
                        String.format("owner %d", i),
                        String.format("restaurant %d", i),
                        5,
                        new RestaurantHours(LocalTime.of(10, 0, 0), LocalTime.of(22, 0, 0)),
    ...
    public static List<RestaurantDto> createRestaurantDtos(List<Restaurant> restaurants) {
        return restaurants.stream()
                .map(restaurant -> {
                    ...
                    hoursDto.open = restaurant.getHours().getOpen().format(DateTimeFormatter.ISO_LOCAL_TIME);
                    hoursDto.close = restaurant.getHours().getClose().format(DateTimeFormatter.ISO_LOCAL_TIME);
                    RestaurantReservationsDto reservationsDto= new RestaurantReservationsDto();
